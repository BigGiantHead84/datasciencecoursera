best <- function(state, outcome) {
    ...
    if(is.na(stateresult)){
        ...
        minimum <- min(newdata$Rate,na.rm = TRUE) #find the minimum value and store it in own variable
        result <- newdata[newdata$Rate==minimum,] #Extract hospital name, state and rate based on minimum value
        print(paste("Hospital with lowest 30 day mortality rate of ",outcome))
        print(result[,1])
    }
}